    ...
    public function mail(Request $request)
    {
        ...
        foreach ($email_addr as $email) {
            ...
            try {
                ...
                {
                    Mail::to($sendToEmail)->send(new ReconMail($dataEmail));
    ...
    public function changestatus($entity_cd='',$project_no='', $trx_type='', $doc_no='', $user_id='', $level_no='', $status='', $profile='', $flag='', $entity_name='', $logo='', $module='')
    {
        ...
        if(count($query)>0) {
            ...
            if($status == 'A') {
                ...
                $sth = $pdo->prepare("SET NOCOUNT ON; EXEC mgr.x_approval_mail_recon ?, ?, ?, ?, ?, ?, ?, ?, ?;");
                ...
                return view('email/recon/revision', $dataRevision);
            }
        }
    }
